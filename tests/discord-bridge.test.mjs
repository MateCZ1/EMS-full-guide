...
const validReport = {
  ...
  responderDepartment: "Fire Department",
  sex: "Muž",
  location: "Los Santos",
...
test("Discord bridge sends a report with a full text attachment", async () => {
  ...
  try {
    ...
    assert.equal(payload.embeds[0].fields[0].value, "Alex Stone");
    assert.equal(payload.embeds[0].fields[1].value, "Fire Department");
    assert.equal(payload.embeds[0].fields[2].value, "John Doe");
